...
from configs.config_isprs_vaihingen import *

from lib.lr_schd import init_params_lr, adjust_learning_rate
from lib.measure import *
from lib.visual import *
from tools.isprs_models import load_model

cudnn.benchmark = True


train_args = IsprsConfigs(net_name='HPAF_Net_rx50_IRRG_DSM_v2',
                          data='Vaihingen',
                          bands_list=['IRRG', 'DSM'],
                          kf=9, k_folder=10,
                          note='reproduce'
...
train_args.input_size = [384, 384]
train_args.val_size = [448, 448]    # [256, 256]
train_args.train_samples = 5000
train_args.train_batch = 5
train_args.val_batch = 5

train_args.hidden_ch = 24 #24
train_args.kernel = 6 #6
train_args.dropout = 0.3
train_args.ndvi = False
train_args.m_views = 3
# 8.5e-5 / np.sqrt(2.0)
train_args.lr = 1.80e-4 # 2.5e-4 / np.sqrt(2.0) #
train_args.steps = 5
train_args.gamma = 0.75
...
def main():
    ...
    # lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 100, 1.18e-7)
